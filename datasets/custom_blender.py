...
class RotBlenderDatasetBase:
    def setup(self, config, split):
        ...
        self.directions = get_ray_directions(
            self.w, self.h, self.focal_x, self.focal_y, self.w // 2, self.h // 2
        ).to(self.rank)  # (h, w, 3)
        ...
        for i, frame in enumerate(meta["frames"]):
            ...
            try:
                ...
                if config.mask_level > 0:
                    mask_operation_size = 3
                    element = cv2.getStructuringElement(
                        cv2.MORPH_RECT, (mask_operation_size, mask_operation_size)
                    )

                    mask = ((np.array(img)[:, :, -1] / 255.0) > 0.5).astype(np.float32)

                    for i in range(config.mask_level):
                        mask = cv2.morphologyEx(
                            mask,
                            cv2.MORPH_DILATE,
                            element,
                            iterations=config.mask_level,
                        )
                    mask = TF.to_tensor(mask)

                    self.all_fg_masks.append(mask[0])
...
class RotBlenderDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        if stage in [None, "fit"]:
            self.train_dataset = RotBlenderIterableDataset(
        ...
        if stage in [None, "predict"]:
            self.predict_dataset = RotBlenderDataset(
                self.config, self.config.train_split
            )
