...
class NeuSModel(BaseModel):
    ...
    def forward_bg_(self, rays):
        ...
        def rgb_sigma_fn(t_starts, t_ends, ray_indices):
            ...
            return rgb, density  # (n_samples, 3), (n_samples, )

        # z_vals_outside = torch.linspace(1e-3, 1.0 - 1.0 / (self.num_samples_per_ray_bg + 1.0), self.num_samples_per_ray_bg, dtype=rays_o.dtype, device=rays_o.device)

        # z_vals_outside = self.far_plane_bg / torch.flip(z_vals_outside, dims=[-1]) + 1.0 / self.num_samples_per_ray_bg

        # z_vals_outside = torch.cat([z_vals_outside.new_tensor([self.far_plane_bg]), z_vals_outside], dim=0)

        # t_starts = z_vals_outside[:-1].repeat(n_rays)
        # t_ends = z_vals_outside[1:].repeat(n_rays)
        ...
        comp_rgb, opacity, depth, extras = nerfacc.rendering(
            t_starts,
            t_ends,
            ray_indices,
            n_rays=n_rays,
    ...
    def forward_(self, rays):
        ...
        if t_dirs.shape[0] == 0:
            rgb = torch.zeros((0, 3), device=rays.device)
        else:
