...
class MeshEvaluator(object):
    ...
    def eval_mesh(
        ...
        Args:
            ...
            pointcloud_tgt (np.ndarray): target point
            normals_tgt (np.ndarray): target normals
            points_iou (np.ndarray): points tensor for IoU evaluation
            occ_tgt (np.ndarray): GT occupancy values for IoU points
            remove_wall (bool, optional): _description_. Defaults to False.
        """
        if len(mesh.vertices) != 0 and len(mesh.faces) != 0:
            if remove_wall: # Remove container boundaries
    ...
    def eval_pointcloud(
        ...
        """ Evaluates a point cloude.

        Args:
            pointcloud (np.ndarray): predicted point cloud
            pointcloud_tgt (np.ndarray): target point cloud
            normals (np.ndarray, optional): predicted normals. Defaults to None.
            normals_tgt (np.ndarray, optional): target normals. Defaults to None.
            thresholds (np.ndarray, optional): threshold values for the F-score calculation. 
        """
        ...
        # Accuracy: how far are the points of the predicted pointcloud from the target point cloud
...
def distance_p2p(points_src, normals_src, points_tgt, normals_tgt):
    ...
    Args:
        ...
        normals_src (np.ndarray): source normals
